...
import { TrashIcon, XIcon } from "@restauwants/ui/icons";
...
export function CollectionList(props: {
  ...
  const { data: collections } = api.collection.all.useQuery(undefined, {
    initialData,
  });
...
export function CollectionCard(props: { name: string; id: number }) {
  ...
  const savePlaceDetailsToState = (details: PlaceDetails | null) => {
            ...
            {APIkey && (
              <Autocomplete
                ...
                onPlaceSelected={(place) => {
                  savePlaceDetailsToState(place as PlaceDetails);
                }}
                options={{
...
export function AddRestaurantToCollectionForm(props: {
  ...
  const utils = api.useUtils();
  const addRestaurant = api.collection.addRestaurant.useMutation({
    onSuccess: async () => {
      form.reset();
      await utils.collection.invalidate();
    },
...
export function AddRestaurantRestaurantForm(props: {
  ...
  const utils = api.useUtils();
  const addRestaurant = api.restaurant.create.useMutation({
    onSuccess: async () => {
      form.reset();
      await utils.collection.invalidate();
    },
...
export function CollectionRestaurantList(props: { collectionId: number }) {
  const { data: restaurants } = api.collection.getCollection.useQuery({
...
export function CollectionRestaurantCard(props: CollectionRestaurantCardProps) {
  const { data: restaurant } = api.collection.getRestaurantName.useQuery({
    restaurantId: props.restaurantId,
  });
  const utils = api.useUtils();
  const deleteCollectionRestaurant =
    api.collection.deleteRestaurant.useMutation({
      onSuccess: async () => {
        await utils.collection.invalidate();
      },
      onError: (err) => {
